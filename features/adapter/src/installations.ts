...
): Promise<AdapterInstallationState> {
...
export async function getAdapterInstallationStates(adapters: AdapterCatalogEntry[]) {
  ...
  return await Promise.all(
    adapters.map((adapter) => getAdapterInstallationState(adapter, installedAdapterIds)),
  )
}

export async function installAdapter(adapter: AdapterCatalogEntry) {
  if (adapter.source === "config") {
    return await getAdapterInstallationState(adapter, await readInstalledAdapterIds())
  }
  ...
  return await getAdapterInstallationState(adapter, installedAdapterIds)
}

export async function uninstallAdapter(adapterId: string) {
  const installedAdapterIds = await readInstalledAdapterIds()
  installedAdapterIds.delete(adapterId)
  await writeInstalledAdapterIds(installedAdapterIds)
  await removeBinaryInstalls(adapterId)
}
