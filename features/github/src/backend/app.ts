...
import { signGitHubWebhookBody, type GitHubWebhookRequestInput } from "./events.ts"
...
export type GoddardGitHubApp = {
  app?: App
  handleWebhook(input: GitHubWebhookRequestInput): Promise<GitHubWebhookResult>
}
...
export function createGitHubApp(options: GitHubAppOptions): GoddardGitHubApp {
  ...
  const handleWebhook = async (input: GitHubWebhookRequestInput): Promise<GitHubWebhookResult> => {
