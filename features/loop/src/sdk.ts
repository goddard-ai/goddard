  ...
  wrap({ client }) {
    return {
      loop: {
        ...
        start: (input: StartLoopRequest) => client.loop.start({ body: input }),

        /** Fetches one daemon loop runtime and its resolved config. */
        get: (input: GetLoopRequest) => client.loop.get({ body: input }),
        ...
        shutdown: (input: ShutdownLoopRequest) => client.loop.shutdown({ body: input }),
      },
    }
  },
})
