...
The agent feature owns launchable ACP agent discovery and runtime readiness.
...
managed install update policy are one product capability: deciding which agents can be
...
- Managed install status and process-spec resolution for acp-client managed-install agents.
- Managed install usage tracking and proactive update scheduling.
- Agent daemon IPC, SDK namespace, schemas, tests, and docs.
...
Session consumes the agent daemon extension for launch process resolution instead
