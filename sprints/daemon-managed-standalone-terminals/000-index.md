...
Deliver daemon-managed terminal instances that are owned by the desktop Bun host, controlled through daemon HTTP requests and daemon streams, scoped by connection-local instance ids, disposed when that host disconnects, and surfaced in the app as a real standalone terminal tab with shared terminal state instead of the current debug-only local fixture shell.
...
- Review `030-add-daemon-terminal-websocket-and-sdk-surface` as the public daemon request/stream control-surface checkpoint.
