...
- `Terminal Instance`
  - A daemon-managed PTY identified by a client-chosen id that is local to one terminal websocket connection.
  - Why: so terminal control frames cannot address terminals created by another client connection.
- `Terminal Lifecycle Event`
  - A daemon-to-client websocket event describing terminal creation, output, process exit, title, cwd, or runtime errors.
