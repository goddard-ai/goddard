...
Daemon plugins may declare in-process `events` they own. A plugin can emit only its own declared events, and it can listen only to its own events plus events declared by plugins in `consumes`.
...
export const inboxPlugin = definePlugin({
  ...
  setup({ events }) {
    events.on("session.turn.ended", handleTurnEnded)
  },
})
```

The `provides` map returned by `setup()` is the feature-owned extension surface consumed by other feature packages. It can contain methods or other typed feature capabilities, but it should not expose feature-private implementation details such as manager instances. Use declared `events` for in-process lifecycle notifications instead of feature-provided event channels. `setup(context)` parameters are inferred from daemon substrate plus the plugin's declared `consumes`, `events`, `db`, `backendRoutes`, `config`, and `ipcRoutes`.

Composition roots use `composePlugins()` after statically importing the feature daemon entrypoints. Composition validates that feature names are unique, every consumed feature is present, feature dependencies are acyclic, event names are unique, route fragments do not collide, and config fragments are grouped under the contributing feature name.
