...
- They keep install and update authority explicit while letting launch flows show managed install status.

## What the daemon decides

- Whether a managed-install agent may be installed before use.
- Whether proactive update behavior applies after recent use.
- Whether managed install state should be surfaced in agent launch listings.

## What the ACP client layer owns

- Managed install metadata.
- Update checks.
- Runnable process resolution for managed-install agents.

## Launch visibility

- A managed-install agent can appear in launch listings even without a Goddard launch-visibility marker.
