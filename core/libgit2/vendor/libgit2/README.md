...
This folder owns the native libgit2 build pipeline for `@goddard-ai/libgit2`. It fetches a pinned libgit2 source release, builds a target-specific shared library, verifies that Bun FFI can load it, and exposes the resulting artifact path for daemon packaging.
