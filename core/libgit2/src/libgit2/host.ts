...
import type { GitApi, GitHistoryApi, GitRefsApi, GitRepositoryApi } from "../types.ts"
import {
...
export const git = defineGitNamespaces({
  repository: createRepositoryApi,
  refs: createRefsApi,
...
function createRepositoryApi(libgit2: Libgit2Symbols): GitRepositoryApi {
  return {
    resolveRoot: (cwd) =>
    ...
    resolveGitPath: (cwd, gitPath) => unsupportedGit.repository.resolveGitPath(cwd, gitPath),
    isBareRepository: (cwd) => unsupportedGit.repository.isBareRepository(cwd),
  }
}

function createRefsApi(libgit2: Libgit2Symbols): GitRefsApi {
  return {
    resolve: (cwd, refName) => resolveRefWithLibgit2(libgit2, cwd, refName),
    exists: async (cwd, refName) =>
      (await createLibgit2RefResolver(libgit2, cwd, refName)) !== null,
    update: (cwd, refName, oid) => unsupportedGit.refs.update(cwd, refName, oid),
    delete: (cwd, refName) => unsupportedGit.refs.delete(cwd, refName),
    ...
    getBranchHead: (cwd, branch) => resolveRefWithLibgit2(libgit2, cwd, `refs/heads/${branch}`),
  }
}

function createHistoryApi(libgit2: Libgit2Symbols): GitHistoryApi {
  return {
    resolveHead: (cwd) => resolveRefWithLibgit2(libgit2, cwd, "HEAD"),
