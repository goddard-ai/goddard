...
the route tree, event definitions, event sources, and provider capabilities consumed by this
package.

Auth sessions are provider-neutral. Provider packages resolve upstream login or installation facts
into backend principals, and daemon event streams authenticate with backend tokens instead of raw
provider tokens. Stream fanout is keyed by provider-neutral principal ids.
...
publication validation, provider capability dispatch, authorization calls, and stream fanout.
Publishing validates that the source may produce the event, validates the provider-agnostic payload
schema, authorizes the source event for the backend principal, then applies the shared
event-envelope filter matcher. Requested stream filters are bandwidth hints, not security
boundaries.
