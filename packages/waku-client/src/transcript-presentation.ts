...
export function formatMessageTime(
  ...
  if (sameLocalDate(date, yesterday)) {
    // Hermes (React Native) ships Intl.DateTimeFormat but not
    // Intl.RelativeTimeFormat; fall back to the English label there.
    const label = typeof Intl.RelativeTimeFormat === 'function'
...
function language(locale?: Intl.LocalesArgument) {
  ...
  // Hermes (React Native) has no Intl.Locale; the BCP-47 primary language
  // subtag is everything before the first "-", which is all we compare on.
  if (typeof Intl.Locale === 'function') return new Intl.Locale(resolved).language
...
function activityNoun(kind: ActivityItem['kind'], count: number) {
  const nouns: Record<ActivityItem['kind'], [string, string]> = {
...
function activityNounKey(kind: ActivityItem['kind'], count: number) {
  const keys: Record<ActivityItem['kind'], [string, string]> = {
