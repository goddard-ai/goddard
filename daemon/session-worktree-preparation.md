...
Fresh isolated sessions can prepare a newly created worktree for agent execution without requiring each repository or operator to repeat the same manual bootstrap work.

## Participants
- Repository Maintainer — declares shared preparation intent for the repository.
- Operator — launches or approves sessions that may use isolated worktrees.
...
## Capabilities
- A fresh isolated worktree may be prepared before the agent starts.
...
- Preparation policy is repository intent so collaborators can share a zero-config default for common setup needs.
...
- User-scoped executable worktree extensions remain a separate trust boundary from repository preparation intent.
