...
Use this skill for product UI work in apps that use Panda CSS. Prefer under-designed over over-designed. Produce correct, accessible, compact UI that fits the existing system and is easy for humans to reshape.
...
- Use the separate `panda-css` skill when the task is library-level setup, troubleshooting, or API-specific investigation rather than product-surface composition.
...
- Do not rebuild dialogs, menus, popovers, tabs, comboboxes, switches, radios, selects, accordions, or tooltips with raw `div`s, custom ARIA, or ad hoc behavior when the project already has an appropriate component or native element.
