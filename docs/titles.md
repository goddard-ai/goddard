...
How each session in the sidebar gets its name: who writes the title, how Goddard
...
UI, on its own cheap model, as part of work it is doing anyway. Goddard's job is
...
2. **Polled from a native store.** The agent writes the title to disk (or to a
   ...
   backoff schedule, latching `RESOLVED` on the first hit. Claude, Amp, Grok,
   and Devin work this way.
3. **Generated by Goddard.** Only Codex.
...
| Codex CLI | **Goddard** (`gpt-5.6-luna`, effort `low`) | second `codex app-server` process | first `turn/started`, new threads only | [codex.rs:636](../crates/waku-core/src/driver/codex.rs#L636) |
...
`generate_session_title` to Sonnet while the turn stayed on Opus. Goddard does not
...
Goddard writes when it forks a session — and it wins by being later in the file.

Goddard reads it on two paths:
...
### Codex — the one Goddard generates

The Codex app-server **persists** a thread name but never generates one; naming
is the client's job, and Codex Desktop does it client-side too. So Goddard matches
that behavior in
...
Devin generates a descriptive title itself (changelog: "automatic generation of
descriptive session titles") and writes it to the CLI session store,
`~/.local/share/devin/cli/sessions.db` (or the equivalent
`dirs::data_local_dir` path). The first prompt is stored there immediately as a
placeholder; the generated name lands after `session/prompt` returns, typically
a few hundred milliseconds later.

ACP `session/load` replays the stored title as `session_info_update`, and Goddard
forwards that when the value is **not** the first-prompt placeholder. A live
first turn does not push that update, so
[`start_devin_title_refresh`](../crates/waku-core/src/driver/acp.rs) polls the
sqlite store with `NativeTitleRefresh` from the first prompt and again at turn
end. Lookups that still see the prompt, Goddard's seven-word fallback, empty, or
`"Untitled"` are misses — latching any of those would hide the generated title
that replaces them.

`_cognition.ai/session/rename` is handled the same way as a generated title if
Devin sends it. Title generation can fail upstream (`agent_ext::title` logs
`failed_precondition`); that is silent, and the truncated-prompt fallback
holds until a distinct title is written.
...
the start of the first turn and Goddard forwards the `title` as `AutoTitleUpdated`,
no polling and no extra process.

What it sends, though, is **the first prompt verbatim** — the same content as
Goddard's own fallback, arriving through a different door. Kimi files it in its
...
3. **Does it only write the title to a native store?** Use `NativeTitleRefresh`.
   ...
   response (Amp, Grok). Devin does both: the generator finishes after the
   turn, but the poll starts with the first prompt so a slow first turn still
   picks the title up.
