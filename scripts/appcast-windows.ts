...
//
// <assets-dir> holds this release's `Goddard-<version>-<arch>-Setup.exe` files.
...
<rss xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle" version="2.0">
  <channel>
    <title>Goddard (Windows ${arch})</title>
${entries}
  </channel>
</rss>
`;
...
export async function generateWindowsAppcasts(
  ...
  for (const arch of architectures) {
    const installer = `Goddard-${version}-${arch}-Setup.exe`;
  ...
  if (written.length === 0) {
    throw new Error(`No Goddard-${version}-<arch>-Setup.exe found in ${assetsDir}`);
  }
  return written;
}
