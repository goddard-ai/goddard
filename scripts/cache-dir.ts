...
// re-derivable, so it lives in the OS cache directory; GODDARD_CACHE_DIR
// overrides the location (CI, sandboxed builds).
import { homedir } from "node:os";
import { join, resolve } from "node:path";

export function wakuCacheDir(): string {
  if (process.env.GODDARD_CACHE_DIR) return resolve(process.env.GODDARD_CACHE_DIR);
  const home = homedir();
  // Not Caches/Waku: on case-insensitive filesystems that is the app's own
  // runtime cache directory.
  if (process.platform === "darwin")
    return join(home, "Library", "Caches", "goddard-build");
  if (process.platform === "win32")
    return join(
      process.env.LOCALAPPDATA ?? join(home, "AppData", "Local"),
      "goddard-build",
    );
  return join(process.env.XDG_CACHE_HOME ?? join(home, ".cache"), "goddard-build");
}
