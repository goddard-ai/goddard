...
/// The shared entrance for a floating surface — menu, popover, or dialog:
/// a bare fade with no movement or resize. The drop shadow rides the same
/// opacity: the shadow shader already cuts the silhouette out of the card
/// interior, so a translucent card never lets it read through and no clip is
/// needed to keep it off the fade. Driven by `with_animation`; under
/// reduce-motion the oneshot delta is 1 and it renders settled.
