...
use super::model_picker::{
    ...
    picker_section_rail_item, picker_selected_row_index, policy_row_parts,
...
/// A collapsed text paste past this size stops being an inline atom and is
...
/// One inline atom folded into the composer — the payload its
/// [`INLINE_ATOM_MARKER`] sentinel stands for. `marker` is the marker's
/// byte offset in the field's content, reseated by [`remap_marker_seats`]
/// after every [`ComposerSplice`]; deleting the marker deletes the atom.
/// Atoms stay ordered by marker offset — the order they appear in the text.
#[derive(Clone, Debug)]
...
#[derive(Clone, Debug)]
pub(super) enum ComposerAtomKind {
    /// A collapsed paste: the raw text, spliced back verbatim.
    PastedText(String),
    /// A session reference: spliced to the `[session ...]` token.
    SessionRef {
        session_id: Uuid,
        title: SharedString,
    },
...
impl ComposerInlineAtom {
    /// What the marker substitutes to in the painted text — mention-style.
    ...
    /// What the marker splices to in the provider-facing prompt.
    ...
    /// The session this atom references, when it is one.
...
impl Waku {
    ...
    pub(super) fn render_model_picker_panel(
        ...
        let available_rows = Rc::new(self.composer_picker_rows(&normalized_query));
        ...
        let section_rows = if searching {
            Rc::new(self.composer_picker_rows(""))
        } else {
        ...
        let list_state = self.model_picker.list.clone();
        let scrollbar_state = self.model_picker.scrollbar.clone();
        let session_selection = self.model_picker_selection(cx);
        let auto_route = self.composer_picker_auto_route();
        ...
        let render_row = Rc::new({
            ...
            let render = move |_row_index: usize,
                  ...
                  -> AnyElement {
                let theme = Theme::current(cx);
                if let PickerRow::Policy(policy) = row {
                    ...
                    let auto = matches!(policy, PolicyRowId::Auto);
                    let (mark, title, subtitle) = policy_row_parts(*policy, &theme);
                    let element = model_picker_row_shell(
                        SharedString::from(format!("model-row-policy-{policy:?}")),
                        auto && auto_route,
                        is_highlighted,
                        &theme,
                    )
                ...
                if let PickerRow::ProviderDefault(provider) = row {
                    // The composer spec never lists one — class pickers
                    // render their own.
                    return model_picker_row_shell(
                        SharedString::from(format!("model-row-default-{}", provider.id())),
                        false,
                        is_highlighted,
                        &theme,
                    )
                ...
                let PickerRow::Combo(row) = row else {
                    unreachable!("every row kind is covered above")
                };
    ...
    /// The composer's row list: the Auto stance while the draft may route,
    /// then favorites, recents, and provider blocks — combos for the
    ...
    pub(super) fn composer_picker_rows(&self, normalized_query: &str) -> Vec<PickerRow> {
        picker_rows(
            ...
            &PickerRowSpec {
                ...
                granularity: self.model_picker_granularity(),
    ...
    fn move_model_picker_highlight(
        ...
        let seed =
            picker_selected_row_index(selection.as_ref(), self.composer_picker_auto_route(), rows);
        if self
            .model_picker
    ...
    pub(super) fn reveal_selected_picker_model(&self, cx: &App) {
        ...
        let index =
            picker_selected_row_index(selection.as_ref(), self.composer_picker_auto_route(), &rows)
                .unwrap_or(0);
        self.model_picker.reveal(index, rows.len());
    }
    ...
    fn cycle_model_picker_section(&mut self, key: &str, cx: &mut Context<Self>) {
        let rows = self.composer_picker_rows("");
        // Seed from the session combo's row the way the reveal lands, so the
        // first tab steps relative to the selection rather than an end.
        let selection = self.model_picker_selection(cx);
        let seed =
            picker_selected_row_index(selection.as_ref(), self.composer_picker_auto_route(), &rows)
                .unwrap_or(0);
        self.model_picker.cycle_section(&rows, seed, key, cx);
    }

    /// Take the row the selection is on, defaulting to the first so `enter`
    /// works the moment the panel opens.
    fn choose_highlighted_model(&mut self, rows: &[PickerRow], cx: &mut Context<Self>) {
        let Some(row) = rows.get(self.model_picker.highlight.unwrap_or(0)) else {
            return;
        };
        match row {
            PickerRow::Policy(PolicyRowId::Auto) => self.choose_auto_route(cx),
            PickerRow::Combo(row) => {
            ...
            // The composer spec lists neither stance.
            PickerRow::Policy(PolicyRowId::NoOverride) | PickerRow::ProviderDefault(_) => {}
        }
    }
    ...
    /// attachment; under it the paste folds into the field as an inline
    ...
    /// [`remap_marker_seats`] for the seating rules — then keep the painted
    /// labels in step with the survivors.
    ...
    pub(super) fn restore_composer_submission(
        ...
        // typed text rather than at their original positions.
...
/// atoms' recorded positions (pre-splice unless the splice itself seated
/// them — `insert_inline_marker` records the marker's post-splice offset);
/// `live` is the content's marker positions, sorted, after the splice;
/// `removed`/`inserted` describe the
...
/// Splice each [`INLINE_ATOM_MARKER`] in `content` back to its atom's
/// payload, in marker order — pasted text verbatim, a session atom's token.
/// The result is what the prompt would read had nothing folded. An
/// unclaimed marker (its atom is already gone) splices to nothing; an atom
/// without a marker folds onto the end, split off by a blank line the way
/// collapsed pastes used to join wholesale.
pub(super) fn splice_inline_atoms(content: &str, atoms: &[ComposerInlineAtom]) -> String {
