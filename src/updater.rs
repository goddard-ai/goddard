...
//! itself with a production build. `GODDARD_PREVIEW_UPDATE=1` fakes only the
//! automatic sidebar result while retaining the real Sparkle flow for the
//! Check for Updates menu; `GODDARD_FORCE_UPDATER=1` exercises everything for
    ...
    impl Updater {
        ...
        pub fn init() -> Option<Self> {
            let preview = cfg!(debug_assertions)
                && std::env::var_os("GODDARD_PREVIEW_UPDATE").is_some_and(|value| value == "1");
            let forced =
                std::env::var_os("GODDARD_FORCE_UPDATER").is_some_and(|value| value == "1");
...
<rss xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle" version="2.0">
  <channel>
    <item>
      ...
      <enclosure url="https://releases.goddardai.org/Goddard-0.1.4-x86_64-Setup.exe" length="1024" type="application/octet-stream" sparkle:edSignature="oldsig" />
    </item>
    <item>
      <title>0.2.0</title>
      <sparkle:shortVersionString>0.2.0</sparkle:shortVersionString>
      <enclosure url="https://releases.goddardai.org/Goddard-0.2.0-x86_64-Setup.exe" length="2048" type="application/octet-stream" sparkle:edSignature="newsig" />
    </item>
  </channel>
</rss>"#;
...
mod windows {
    ...
    const FEED_URL: &str = "https://releases.goddardai.org/appcast-windows-aarch64.xml";
    #[cfg(not(target_arch = "aarch64"))]
    const FEED_URL: &str = "https://releases.goddardai.org/appcast-windows-x86_64.xml";
    ...
    impl Updater {
        ...
        fn start_check(&self, user_initiated: bool) {
            ...
            let spawned = std::thread::Builder::new()
                .name("goddard-updater-check".into())
                .spawn(move || {
        ...
        pub fn set_automatically_checks_for_updates(&self, enabled: bool) {
            ...
            let _ = std::thread::Builder::new()
                .name("goddard-updater-preference".into())
    ...
    mod tests {
        ...
        fn an_absent_preference_file_leaves_automatic_checks_on() {
            let directory = std::env::temp_dir()
                .join(format!("goddard-updater-preference-{}", std::process::id()));
            let _ = std::fs::remove_dir_all(&directory);
