...
- Start each overview page with one short Markdown blockquote that explains what the concept is and why it matters.
...
- `overview/` may diverge from `spec/` when the spec describes an ideal or future shape; prefer the current product shape in overview docs.
...
- Agents still must verify that overview prose is public-facing, conceptually useful, terminology-aligned, and accurate to current supported behavior.
