...
- Use `git commit --amend` only for obvious nitpicks or when the user explicitly requests an amend; an explicit amend request applies only to the current agent turn and does not carry over.
